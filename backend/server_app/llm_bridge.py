...
llama-3.3-70b-versatile by default — same client already used for lead
    ...
    for hop in range(_MAX_TOOL_HOPS):
        try:
            stream = await groq_client.chat.completions.create(
                model=GROQ_MODEL,   # MIGRATION: llama-3.3-70b-versatile by default, called directly on Groq
                messages=messages,
                tools=_TOOLS,
                stream=True,
            )
